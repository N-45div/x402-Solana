...
function requirePayment(amount, description, resource, acceptSOL = true, acceptUSDC = true) {
    return async (req, res, next) => {
        ...
        if (!paymentHeader) {
            ...
            const accepts = [];
            if (acceptSOL) {
                accepts.push((0, core_1.createSolTransferRequirement)(PAYMENT_ADDRESS, amount, resource, description, 'solana-devnet'));
            }
            if (acceptUSDC) {
                accepts.push((0, core_1.createSplTransferRequirement)(PAYMENT_ADDRESS, amount, core_1.USDC_MINT_DEVNET, resource, description, 'solana-devnet'));
            }
        ...
        try {
            ...
            const requirement = acceptSOL && paymentPayload.scheme === 'solana-transfer'
                ? (0, core_1.createSolTransferRequirement)(PAYMENT_ADDRESS, amount, resource, description, 'solana-devnet')
                : acceptUSDC && paymentPayload.scheme === 'solana-spl'
                    ? (0, core_1.createSplTransferRequirement)(PAYMENT_ADDRESS, amount, core_1.USDC_MINT_DEVNET, resource, description, 'solana-devnet')
                    : null;
            if (!requirement) {
            ...
            const verifyResponse = await axios_1.default.post(`${facilitatorUrl}/verify`, {
                x402Version: 1,
                paymentHeader,
            ...
            // Settle payment
            const settleResponse = await axios_1.default.post(`${facilitatorUrl}/settle`, {
                x402Version: 1,
                paymentHeader,
                paymentRequirements: requirement
            });
...
app.get('/api/market-data', requirePayment('0.01', 'Premium market data access', '/api/market-data'), (req, res) => {
    res.json({
...
app.post('/api/ai-analysis', requirePayment('0.05', 'AI-powered analysis service', '/api/ai-analysis'), (req, res) => {
...
app.get('/api/premium-feed', requirePayment('0.1', 'Premium real-time data feed', '/api/premium-feed'), (req, res) => {
    res.json({
...
// SOL-only endpoint
app.get('/api/sol-only', requirePayment('0.02', 'SOL-only premium content', '/api/sol-only', true, false), (req, res) => {
    res.json({
...
// USDC-only endpoint
app.get('/api/usdc-only', requirePayment('0.03', 'USDC-only premium content', '/api/usdc-only', false, true), (req, res) => {
    res.json({
